                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{ route('admin.authors.update', $author) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            @if($author->image)
                                <div class="mt-2 mb-4">
                                    <img src="{{ asset('storage/' . $author->image) }}" alt="{{ $author->name }}" class="h-48 w-auto object-cover rounded-lg shadow-sm">
                                    <p class="text-sm text-gray-500 mt-1">تصویر فعلی</p>
                                </div>
                            @endif
                            <input type="file" name="image" id="image" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3">
                            <p class="text-gray-500 text-xs mt-1">برای تغییر تصویر، فایل جدید انتخاب کنید. در غیر این صورت، تصویر فعلی حفظ می‌شود.</p>
                            @error('image')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
