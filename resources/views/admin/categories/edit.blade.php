            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <div class="mb-6 bg-yellow-50 p-4 rounded border border-yellow-200">
                        <h3 class="font-bold text-lg mb-2">اطلاعات فعلی دسته‌بندی:</h3>
                        <p><strong>شناسه:</strong> {{ $category->id }}</p>
                        <p><strong>نام:</strong> {{ $category->name }}</p>
                        <p><strong>اسلاگ:</strong> {{ $category->slug }}</p>
                        <p><strong>توضیحات:</strong> {{ $category->description ?? 'خالی' }}</p>
                        <p><strong>لینک صفحه:</strong> <a href="{{ route('blog.category', $category->slug) }}" target="_blank" class="text-blue-600">{{ route('blog.category', $category->slug) }}</a></p>
                    </div>
                    ...
                    <form action="{{ route('admin.categories.update', $category) }}" method="POST" id="categoryForm">
                        ...
                        <!-- اسلاگ دسته‌بندی -->
                        <div class="mb-4">
                            <label for="slug" class="block text-sm font-medium text-gray-700">اسلاگ دسته‌بندی</label>
                            <div class="flex items-center mt-1">
                                <span class="text-gray-500 bg-gray-100 px-3 py-2 rounded-r-md border border-l-0 border-gray-300">{{ url('/category/') }}/</span>
                                <input type="text" name="slug" id="slug" value="{{ old('slug', $category->slug) }}" class="flex-1 block w-full rounded-l-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" placeholder="slug-example">
                            </div>
                            <p class="text-gray-500 text-xs mt-1">اگر خالی بگذارید، به صورت خودکار از نام دسته‌بندی ساخته می‌شود.</p>
                            @error('slug')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <!-- توضیحات دسته‌بندی -->
                        <div class="mb-4">
                            <label for="description" class="block text-sm font-medium text-gray-700">توضیحات دسته‌بندی</label>
                            ...
                            <p class="text-gray-500 text-xs mt-1">توضیحات کوتاهی درباره این دسته‌بندی بنویسید.</p>
                            @error('description')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
            ...
            checkFormButton.addEventListener('click', function() {
                ...
                <div class="mt-4 p-3 bg-blue-50 border border-blue-200 rounded">
                    ...
                    <ol class="list-decimal pl-5 mt-2">
                        <li>نام فیلدها در فرم (name, slug, description) دقیقاً با نام فیلدهای مدل مطابقت دارند.</li>
