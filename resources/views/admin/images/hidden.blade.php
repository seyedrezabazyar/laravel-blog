...
<head>
    ...
    <title>گالری تصاویر رد شده | پنل مدیریت</title>
...
<nav class="navbar">
    <div class="container">
        ...
        <ul class="navbar-nav mr-auto">
            ...
            <li class="nav-item">
                <a class="nav-link active-nav" href="/admin/gallery/hidden">تصاویر رد شده</a>
            </li>
        </ul>
    </div>
</nav>
...
<div class="gallery-container">
    <div class="gallery-header">
        <h1>گالری تصاویر رد شده</h1>
        <p>مدیریت تصاویر رد شده</p>
    </div>
    ...
    <div class="counter" id="imageCounter">
        تعداد تصاویر رد شده: <span id="totalCount">0</span>
    </div>
    ...
    <div class="empty-gallery d-none" id="emptyGallery">
        <h3>هیچ تصویر رد شده‌ای موجود نیست</h3>
    ...
    <div class="bulk-actions-container">
        ...
        <div class="d-flex justify-content-center flex-wrap">
            <button class="bulk-btn bulk-btn-unapprove" onclick="bulkManage('unapprove')">بازگشت به حالت اولیه</button>
        </div>
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function createImageCard(image) {
            ...
            // تاریخ آخرین بروزرسانی تصویر
            const updatedDate = image.updated_at ? new Date(image.updated_at) : null;
            const formattedDate = updatedDate ?
                `${updatedDate.toLocaleDateString('fa-IR')} ${updatedDate.toLocaleTimeString('fa-IR', {hour: '2-digit', minute:'2-digit'})}`
                : 'نامشخص';
                ...
                <div class="image-card">
                    <div class="image-container">
                        <img src="${image.raw_image_url}" class="gallery-image" alt="تصویر ${image.id}"
                            ...
                            data-updated-at="${image.updated_at || ''}"
                        ...
                        <div class="rejected-badge" title="تاریخ بروزرسانی: ${formattedDate}">رد شده</div>
                    </div>
                    <div class="image-actions">
                        <button class="btn-action btn-orange" onclick="manageImage(${image.id}, 'unapprove')">بازگشت به حالت اولیه</button>
                    </div>
                </div>
            `;
        ...
        // تابع برای مدیریت تصویر
        window.manageImage = function(imageId, action) {
            if (confirm(`آیا از بازگرداندن این تصویر به حالت اولیه اطمینان دارید؟`)) {
                // نمایش افکت محو شدن
                ...
                fetch('/admin/api/gallery/manage', {
                    method: 'POST',
                    headers: {
                    ...
                    .then(data => {
                        if (!data.success) {
                            console.error('خطا در تغییر وضعیت تصویر:', data.message);
                        }
                    })
                    .catch(error => {
        ...
        // تابع برای مدیریت گروهی تصاویر
        window.bulkManage = function(action) {
            if (confirm(`آیا از بازگرداندن همه تصاویر موجود در این صفحه به حالت اولیه اطمینان دارید؟`)) {
                ...
                Promise.all(requests)
                    .then(results => {
                        console.log(`${results.length} تصویر با موفقیت به حالت اولیه بازگردانده شدند.`);
        ...
        window.openLightbox = function(imgElement) {
            ...
            const updatedAt = imgElement.dataset.updatedAt;
            ...
            lightboxInfo.innerHTML = `
                ...
                <strong>تاریخ بروزرسانی:</strong> ${formattedDate}
            `;
